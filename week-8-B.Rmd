...
$$ att\_chg_i = \beta_1 x_{treated,i} + \beta_2 x_{control,i} + \epsilon_i$$

* Describe the ideal Preceptor Table.

Would have Preceptor's attitude change in both a controlled and a treatment situation.

* Write a few sentences about whether or not our data will allow us to answer this question.

Our data can offer us a picture of the potential outcome, and could be used to
make a prediction. However, the accuracy may not be high as the sample 
population may not be representative of the Preceptor as an individual, so the 
change is likely to be different.
...
preceptor_obj <- stan_glm(data = week_8,
                          family = gaussian(),
                          att_chg ~ treatment - 1,
                          refresh = 0)
...
new_obs <- tibble(treatment = "treated")

pp <- posterior_predict(object = preceptor_obj) %>%    
  as_tibble() %>%    
...
libtrue_obs <- tibble(liberal = TRUE)
libfalse_obs <- tibble(liberal = FALSE)
male_obs <- tibble(gender = "male")
